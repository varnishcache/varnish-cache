...
vcl_output_lang_h(struct vsb *sb)
{
	...
	vsb_cat(sb, "ruct sockaddr * VRT_r_client_ip(const struct sess *);\n");
	vsb_cat(sb, "struct sockaddr * VRT_r_server_ip(struct sess *);\nint");
	vsb_cat(sb, " VRT_r_server_port(struct sess *);\nconst char * VRT_r");
	vsb_cat(sb, "_req_request(const struct sess *);\nvoid VRT_l_req_req");
	vsb_cat(sb, "uest(const struct sess *, const char *, ...);\nconst c");
	vsb_cat(sb, "har * VRT_r_req_url(const struct sess *);\nvoid VRT_l_");
	vsb_cat(sb, "req_url(const struct sess *, const char *, ...);\ncons");
	vsb_cat(sb, "t char * VRT_r_req_proto(const struct sess *);\nvoid V");
	vsb_cat(sb, "RT_l_req_proto(const struct sess *, const char *, ...)");
	vsb_cat(sb, ";\nvoid VRT_l_req_hash(struct sess *, const char *);\n");
	vsb_cat(sb, "struct director * VRT_r_req_backend(struct sess *);\nv");
	vsb_cat(sb, "oid VRT_l_req_backend(struct sess *, struct director *");
	vsb_cat(sb, ");\nint VRT_r_req_restarts(const struct sess *);\ndoub");
	vsb_cat(sb, "le VRT_r_req_grace(struct sess *);\nvoid VRT_l_req_gra");
	vsb_cat(sb, "ce(struct sess *, double);\nconst char * VRT_r_req_xid");
	vsb_cat(sb, "(struct sess *);\nconst char * VRT_r_bereq_request(con");
	vsb_cat(sb, "st struct sess *);\nvoid VRT_l_bereq_request(const str");
	vsb_cat(sb, "uct sess *, const char *, ...);\nconst char * VRT_r_be");
	vsb_cat(sb, "req_url(const struct sess *);\nvoid VRT_l_bereq_url(co");
	vsb_cat(sb, "nst struct sess *, const char *, ...);\nconst char * V");
	vsb_cat(sb, "RT_r_bereq_proto(const struct sess *);\nvoid VRT_l_ber");
	vsb_cat(sb, "eq_proto(const struct sess *, const char *, ...);\ncon");
	vsb_cat(sb, "st char * VRT_r_obj_proto(const struct sess *);\nvoid ");
	vsb_cat(sb, "VRT_l_obj_proto(const struct sess *, const char *, ...");
	vsb_cat(sb, ");\nint VRT_r_obj_status(const struct sess *);\nvoid V");
	vsb_cat(sb, "RT_l_obj_status(const struct sess *, int);\nconst char");
	vsb_cat(sb, " * VRT_r_obj_response(const struct sess *);\nvoid VRT_");
	vsb_cat(sb, "l_obj_response(const struct sess *, const char *, ...)");
	vsb_cat(sb, ";\nint VRT_r_obj_hits(const struct sess *);\nunsigned ");
	vsb_cat(sb, "VRT_r_obj_cacheable(const struct sess *);\nvoid VRT_l_");
	vsb_cat(sb, "obj_cacheable(const struct sess *, unsigned);\ndouble ");
	vsb_cat(sb, "VRT_r_obj_ttl(const struct sess *);\nvoid VRT_l_obj_tt");
	vsb_cat(sb, "l(const struct sess *, double);\ndouble VRT_r_obj_grac");
	vsb_cat(sb, "e(const struct sess *);\nvoid VRT_l_obj_grace(const st");
	vsb_cat(sb, "ruct sess *, double);\ndouble VRT_r_obj_prefetch(const");
	vsb_cat(sb, " struct sess *);\nvoid VRT_l_obj_prefetch(const struct");
	vsb_cat(sb, " sess *, double);\ndouble VRT_r_obj_lastuse(const stru");
	vsb_cat(sb, "ct sess *);\nconst char * VRT_r_obj_hash(const struct ");
	vsb_cat(sb, "sess *);\nconst char * VRT_r_resp_proto(const struct s");
	vsb_cat(sb, "ess *);\nvoid VRT_l_resp_proto(const struct sess *, co");
	vsb_cat(sb, "nst char *, ...);\nint VRT_r_resp_status(const struct ");
	vsb_cat(sb, "sess *);\nvoid VRT_l_resp_status(const struct sess *, ");
	vsb_cat(sb, "int);\nconst char * VRT_r_resp_response(const struct s");
	vsb_cat(sb, "ess *);\nvoid VRT_l_resp_response(const struct sess *,");
	vsb_cat(sb, " const char *, ...);\ndouble VRT_r_now(const struct se");
	vsb_cat(sb, "ss *);\nunsigned VRT_r_req_backend_healthy(const struc");
	vsb_cat(sb, "t sess *);\n");
}
